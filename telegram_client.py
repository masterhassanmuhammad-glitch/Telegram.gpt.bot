...
async def async_get_messages(chat_id, limit=20):
    ...
    if not tg_app:
        return messages
    try:
        history = tg_app.get_chat_history(chat_id, limit=limit)
        async for msg in history:
            try:
                text_content = getattr(msg, "text", None) or getattr(msg, "caption", None) or "[مرفق / وسائط]"
                text_content = str(text_content).encode("utf-8", "ignore").decode("utf-8")
            except Exception:
                text_content = "[نص يحتوي رموز خاصة تعذر قراءتها]"
        ...
        log(f"⚠️ Error fetching chat history: {repr(e)}")
...
async def async_get_single_message(chat_id, message_id):
    ...
    try:
        ...
        try:
            raw_text = getattr(msg, "text", None) or getattr(msg, "caption", None) or "[لا يوجد نص]"
...
async def async_search_messages(query):
    ...
    try:
        async for msg in tg_app.search_global(query, limit=10):
            try:
                raw_text = getattr(msg, "text", None) or getattr(msg, "caption", None) or "محتوى غير نصي"
...
def handle_telegram_command(sender_id, text, gemini_response_fn):
    ...
    if cmd.isdigit():
        if state["state"] in ["VIEWING_CHANNELS", "VIEWING_GROUPS", "VIEWING_CHATS"]:
            idx = int(cmd) - 1
            target = []
            ...
            if 0 <= idx < len(target):
                selected = target[idx]
                log(f"📖 Opening selected chat #{idx+1}: {selected['title']}")
            ...
            return "💡 يرجى استعراض القنوات أو المجموعات أولاً (أرسل `/channels` أو `/مجموعات`) قبل اختيار الرقم."
